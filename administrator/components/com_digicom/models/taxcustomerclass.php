...
class DigiComAdminModelTaxcustomerclass extends JModelList {
	protected $_context = 'com_digicom.Taxcustomerclass';
	...
	function getcustomerClass() {
		if (empty ($this->_cclass)) {
			$this->_cclass = $this->getTable("TaxCustomerClass");
	...
	function store () {
		$item = $this->getTable('TaxCustomerClass');
...
//			$this->setError($item->getErrorMsg());
	...
	function publish () {
		...
		if ($task == 'publish'){
			$res = 1;
			$sql = "update #__digicom_tax_customerclass set published='1' where id in ('".implode("','", $cids)."')";
		} else {
			$res = -1;
			$sql = "update #__digicom_tax_customerclass set published='0' where id in ('".implode("','", $cids)."')";
...
//		return false;
	...
	function shiftorder($direction = 1) {
		...
		$sql = "update #__digicom_tax_customerclass set `ordering`=ordering".($direction == 1?"+1":"-1")." where id=".$cids[0];
		$db->setQuery($sql);
		$res = $db->query();
		if (!$res) return $res;
		return true;
	...
	function reorder () {
		...
		$order = JRequest::getVar('order', array(0), 'post', 'array');
		foreach ($cids as $i => $v) {
			$sql = "update #__digicom_tax_customerclass set `ordering`='".$order[$i]."' where id=".$v;
			$db->setQuery($sql);
			$res = $db->query();
			if (!$res) return $res;
		}
