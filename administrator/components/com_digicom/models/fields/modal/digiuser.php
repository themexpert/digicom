...
//		echo '<pre>'.print_r($this, true).'</pre>';
		...
		$script[] = '	function jSelectDigiUser_' . $this->id . '(id, title) {';
		...
		if ($allowClear && !$scriptClear)
		{
			...
			$script[] = '	function jClearDiGiUser(id) {';
			$script[] = '		document.getElementById(id + "_id").value = "";';
			$script[] = '		document.getElementById(id + "_name").value = "' . htmlspecialchars(JText::_('COM_DIGICOM_FIELD_SELECT_USER_LABEL', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$html	= array();
		$link	= 'index.php?option=com_digicom&amp;controller=customers&amp;layout=modal&amp;tmpl=component&amp;field=' . $this->id;
		...
		if (empty($title))
		{
			$title = JText::_('COM_DIGICOM_FIELD_SELECT_USER_LABEL');
		}
		...
		$html[] = '<a class="modal btn hasTooltip" title="' . JHtml::tooltipText('COM_DIGICOM_CHANGE_CUSTOMMER') . '"  href="' . $link . '&amp;' . JSession::getFormToken() . '=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> ' . JText::_('JSELECT') . '</a>';
		...
		if ($allowClear)
		{
			$html[] = '<button id="' . $this->id . '_clear" class="btn' . ($value ? '' : ' hidden') . '" onclick="return jClearDiGiUser(\'' . $this->id . '\')"><span class="icon-remove"></span> ' . JText::_('JCLEAR') . '</button>';
		}
