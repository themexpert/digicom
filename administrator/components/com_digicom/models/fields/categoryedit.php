...
class JFormFieldCategoryEdit extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		if ($this->element['parent'] || $jinput->get('option') == 'com_categories')
		{
			...
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('extension', 'com_digicom');
		}
		else
			// For items the old category is the category they are in when opened or 0 if new.
		{
			$oldCat = $this->form->getValue($name, 0);
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('option', 'com_digicom');
		}
		...
		else
		{
			 ...
			 */
			//if(is_array($oldCat)) $oldCat = $oldCat[0];
			foreach ($options as $i => $option)
			{
