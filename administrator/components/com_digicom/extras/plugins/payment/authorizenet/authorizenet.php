...
class plgpaymentAuthorizenet extends JPlugin 
{
	...
	function onTP_Processpayment($data) 
	{
		...
		$error = array();
		$action_url = plgPaymentAuthorizenetHelper::buildAuthorizenetUrl($this->params);
		...
		if ($allresp[0] > 1)
		{
			...
			$data = 'activated='.$post['activated'].'&cardnum='.$post['cardnum'].'&cardcvv='.$post['cardcvv'].'&expmonth='.$post['expmonth'].'&expyear='.$post['expyear'].'&cardfname='.$post['cardfname'].'&cardlname='.$post['cardlname'].'&cardaddress1='.$post['cardaddress1'].'&cardcity='.$post['cardcity'].'&cardstate='.$post['cardstate'].'&cardzip='.$post['cardzip'].'&cardcountry='.$post['cardcountry'].'&cardemail='.$post['email'];
			header('Location: index.php?option=com_digicom&controller=cart&task=checkout&processor=authorizenet&'.$data.'&error=1&errormsg='.$allresp[3]);
			exit();
		}
