...
require_once(JPATH_SITE.DS."components".DS."com_digicom".DS."models".DS."digicomconfig.php");
...
$sql = "SELECT `csym`
		...
		WHERE `ccode`='" . $configs->get('currency','USD') . "'";
$db->setQuery($sql);
...
<div class="digicom" style="<?php echo $configs->get('shopping_cart_style',0) ? 'width: 70%;margin-left: auto;margin-right: auto;padding: 20px 0 0 0;background: #fff;' : ''; ?>">
	<div class="container-fluid">

		<?php if($configs->get('shopping_cart_style',0)) {
			echo '<a href="' . JURI::root() . '">
				<img src="' . JURI::root() . 'images/stories/digicom/store_logo/' . trim($configs->get('store_logo',0)) . '" alt="store_logo" border="0">
			</a>';
			}
		?>

		<?php
			if($configs->get('show_steps',1) == 0){
		?>
				<div class="bar">
		...
		<div style="background:#f7f7f7;border-radius:5px;-webkit-border-radius:5px;-moz-border-radius:5px;padding-top:10px;">
			<div class="row-fluid">
				<div class="span6">
					&nbsp;&nbsp;&nbsp;
					<span style="color:#666;">Your Total:</span> <b><?php echo (!$configs->get('currency','USD')_position ? '&#' . $currency_symbol[0] . ';' . ' ' : '') . $vars->amount . ($configs->get('currency','USD')_position ? ' ' . '&#' . $currency_symbol[0] . ';' : '');?></b>
				</div>
				<div class="span6">
					...
					<div class="control-group">
						...
						<div class="controls" style="margin-left:5px;">
							<select id="cardcountry" name="cardcountry" class="inputbox required" onchange="getStates();" tabindex="10" style="width:200px;">
								...
								if ($configs->get('topcountries','') != '')
								{
									$countries_top = $configs->topcountries;
									$where = '';
