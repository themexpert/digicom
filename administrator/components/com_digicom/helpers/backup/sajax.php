...
//changed rsargs[] to rsargs due to errors on news4.ijoomla.com
//if any issues arise - check that 1st
...
//print_r($_SERVER["REQUEST_URI"]);
		$sajax_remote_uri = DigiComAdminHelper::getLiveSite();//$_SERVER["REQUEST_URI"];
		$sajax_remote_uri .= "/index.php?option=com_digicom";
		if (!strpos( $sajax_remote_uri, "?option=com_digicom")) {
			if (strpos( $sajax_remote_uri, "?")) {
...
sajax_init();
sajax_export("phpchangeProvince", "phpchangetaxProvince");
sajax_handle_client_request();


function phpchangeProvince($country, $type = '') {
	 $database = JFactory::getDBO();
	if ($type == 'ship') $prefix = $type;
	else $prefix = '';
//echo JRequest::getVar();
	$query = "SELECT state"
	...
	$state_option = "<select name='".$prefix."state' id='".$prefix."sel_province' >";
	if ($states) {
		foreach( $states as $state ) {
			$state_option .= "<option value='" . $state->state . "'>" . $state->state . "</option>";	
		}
	} else {
			$state_option .= "<option value=''>".(JText::_("SAJAXSELCOUNT"))."</option>";	
...
function phpchangetaxProvince($country, $type = '') {
	 $database = JFactory::getDBO();
	if ($type == 'ship') $prefix = $type;
	else $prefix = '';
//echo JRequest::getVar();
	...
	$states = $database->loadObjectList();
	$state_option = "<select name='".$prefix."state' id='".$prefix."sel_province' >";
			$state_option .= '<option value="All" ';
			$state_option .= 'selected';
			$state_option .= '>'.(JText::_('DSALL')).'</option>';		 
