...
(function($) {
	$.fn.repeatable_fields = function(custom_settings) {
		...
		function after_add(container, new_row) {
			...
			$('*', new_row).each(function() {
				$.each(this.attributes, function(index, element) {
					...
					this.value = this.value.replace(/_row_count_placeholder_id_/, row_count - 1);
				});
			});
		...
		function before_remove(container, row) {
			...
			var jform_files_remove_id = $("#jform_files_remove_id").val();
			if(jform_files_remove_id){
				$('#jform_files_remove_id').val(jform_files_remove_id + ',' + filesId);
			}else{
				$('#jform_files_remove_id').val(filesId);
			} 
		}
	}
})(jQuery);