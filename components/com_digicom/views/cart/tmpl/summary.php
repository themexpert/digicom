...
//		$select_only = $this->select_only;//array();
	...
	$invisible = 'style="display:none;"';
		if ( count ( $items ) == 0 ) {
			...
			<input type="button"  class="button" name="continue" value="<?php echo JText::_("DSCONTINUESHOPING");?>" onClick="window.location='<?php
				echo DigiComHelper::DisplayContinueUrl($configs,$cat_url);
			 ?>';">
			</form>
	...
	$cart_url = JRoute::_("index.php?option=com_digicom&controller=cart&task=view&Itemid=".$Itemid);
	$checkout_url = JRoute::_("index.php?option=com_digicom&controller=cart&task=checkout&fromsum=1&Itemid=".$Itemid);
//print_r($customer);
		?>
...
<h1><?php echo JText::_("DSSUMMARY")?></h1>
	...
	if ($configs->showreplic != "1" || ($configs->showreplic == "1" && $totalfields < 1))
	  ...
	  <td class="item_column" style="text-align:center; <?php if ($configs->get('showcam',1) == 0) echo ' display:none;';?>" >
	...
	<td class="item_column" style="text-align:center;"> <?php
		echo  DigiComHelper::format_price($item->price, $item->currency, true, $configs);//sprintf($price_format,$item->product_price)." ".$item->currency;
	?></td>
...
echo (isset($item->shipping)&&$item->domainrequired==2? DigiComHelper::format_price($item->shipping, $item->currency, true, $configs):"N/A");
	?></td>
	...
	?></td>
	  ...
	  /*
									if ($configs->get('tax_price','') == 1)
										...
										echo  DigiComHelper::format_price($item->price, $item->currency, true, $configs);
	   */
			echo  DigiComHelper::format_price($item->subtotal, $item->currency, true, $configs);
//print_r($item);
	  ?>
...
<?php
	  ...
	  <td class="item_column_right" nowrap style="text-align:center; padding-top:10px;">
		...
		<?php
				if ($configs->tax_summary == 1){
										if ($tax['promo'] > 0 && $tax['promoaftertax'] == '0')
											echo DigiComHelper::format_price($tax['promo'], $tax['currency'], true, $configs)."<br />";
											//echo $tax['promo']." ".$tax['currency'];
										?>
														  <?php
										if ($tax['value'] > 0 && $customer->_user->id > 0)
											echo DigiComHelper::format_price($tax['value'], $tax['currency'], true, $configs)."<br />";
												//echo $tax['value']." ".$tax['currency'];
										?>

										<?php
										if ($tax['shipping'] > 0 && $customer->_user->id > 0)
											echo DigiComHelper::format_price($tax['shipping'], $tax['currency'], true, $configs)."<br />";
		...
		?>
	<?php
		  echo DigiComHelper::format_price($tax['taxed'], $tax['currency'], true, $configs)."<br />";
	?></td>
	</tr>
	<tr >
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="2">
		<tr>
		  <td> <input type="button" class="digicom_cancel btn" name="continue" value="<?php echo JText::_("DSEDITORDER")?>" onClick="window.location='<?php echo $cart_url;?>';" <?php if ($configs->showccont == 0) echo $invisible;?>  ></td>
