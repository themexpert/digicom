...
class DigiComModelProducts extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.name, a.alias, a.description, a.fulldescription, ' .
					...
					'a.publish_down, a.images, a.attribs, a.metadata, a.metakey, a.metadesc, a.access, ' .
		...
		// Join over the frontpage products.
		if ($this->context != 'com_digicom.featured')
		{
		...
		$query->select("CASE WHEN a.created_by > ' ' THEN a.created_by ELSE ua.name END AS author")
		...
		$query->select('ROUND(v.rating_sum / v.rating_count, 0) AS rating, v.rating_count as rating_count')
			->join('LEFT', '#__digicom_products_rating AS v ON a.id = v.content_id');
		...
		$articleId = $this->getState('filter.product_id');
	...
	public function getItems()
	{
		...
		foreach ($items as &$item)
		{
			...
			if (!$guest)
			{
				$asset = 'com_digicom.article.' . $item->id;
