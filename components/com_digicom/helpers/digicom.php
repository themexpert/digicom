			...
			foreach ( $item->productfields as $j => $v ) {
				...
				foreach ( $options as $i1 => $v1 ) {
					...
					if ( isset( $val[1] ) && strlen( trim( $val[1] ) ) > 0 ) {
						$r = $val[0] . " - " . ( JText::_( "DSADD" ) ) . " " . ( DigiComSiteHelperDigiCom::format_price( $val[1], $configs->get('currency','USD'), true, $configs ) );
					} else {
						$r = $val[0];
					}
					$res .= ( "<option value='" . $i1 . "'" );
...
//					$theURI .= '?' . $_SERVER['QUERY_STRING'];
	...
	public static function getRelatedItems( $id ) {
		...
		if ( $metakey = trim( $db->loadResult() ) ) {
			$catids = DigiComSiteHelperDigiCom::getCategoryByProductID( $id );
...
// 			$words = DigiComSiteHelperDigiCom::str_word_count_unicode(strip_tags($text), 1);
// 			var_dump($words);
			$i = 0;
