...
<?php if (!$this->configs->get('catalogue', 0) and !$this->item->hide_public) : ?>
	<div class="dc-addtocart-bar">
		<form name="prod" class="form" id="product-form" action="<?php echo JRoute::_('index.php?option=com_digicom&view=cart');?>" method="post" style="width:100%;">
			<div class="form-group<?php echo ($this->configs->get('show_quantity',0) == 1 ? " with-qnty " : ' '); ?>no-padding no-margin">
				...
				<?php if($this->configs->get('afteradditem',0) == "2") {	?>
					<div type="button" class="btn btn-success btn-md btn-cart" onclick="Digicom.addtoCart(<?php echo $this->item->id; ?>,'<?php echo JRoute::_("index.php?option=com_digicom&view=cart"); ?>');"><?php echo JText::_("COM_DIGICOM_ADD_TO_CART");?></div>
				<?php }else { ?>
					<button type="submit" class="btn btn-success btn-lg"> <?php echo JText::_('COM_DIGICOM_ADD_TO_CART'); ?></button>
					<?php } ?>
			</div>
