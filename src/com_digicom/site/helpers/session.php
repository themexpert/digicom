...
class DigiComSiteHelperSession
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        if ($this->_user->id > 0) {
            ...
            if ( ! $table->id) {
                ...
                if ( ! empty($table->id) && $table->id) {
                    // we have found customer by id
        ...
        if ( ! isset($this->_customer->registerDate)) {
            $this->_customer->registerDate = $my->registerDate;
        }
        if ( ! isset($this->_customer->id) && $my->id) {
            $this->_customer->id = $my->id;
        }
    ...
    function getTransactionData()
    {
        ...
        if (empty($this->_sid) || $this->_sid < 1) {
            return null;
        }
        $db  = JFactory::getDBO();
